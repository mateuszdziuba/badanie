...
const genQuiz = list => {
  ...
  answers = answers.concat(
    _.sampleSize(list.words, 7),
    list.answers.critical,
    _.sampleSize(errors, 8)
  );
...
const checkQuiz = list => {
  ...
  for (let i = 0; i < answers.length; i++) {
    if (list.words.indexOf(answers[i]) != -1) {
      ...
      critical++;
    } else if (errors.indexOf(answers[i]) != -1) {
...
const genRaport = () => {
  ...
  const moods = ['pozytywny/radosny', 'negatywny/smutny', 'neutralny'];
